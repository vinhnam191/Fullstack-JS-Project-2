...
productRoute.get("/", async (_req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ message: "Get list of users successfully", data: products })
    } catch (error) {
...
productRoute.get("/:id", async (req: Request, res: Response) => {
    try {
        ...
        if (productId && (typeof productId == 'number')) {
            ...
            res.status(200).json({ message: "Get product info successfully", data: product })
        } else {
...
productRoute.get('/category/:category', async (req: Request, res: Response) => {
    try {
        ...
        if (categoryName) {
            const productByCategory = await productStore.getProductByCategory(categoryName);
            res.status(200).json({
            ...
            res.status(400).send({ message: "Please pass product category that you want to search" })
        }
    } catch (error) {
        res.status(500).json({
            message: `${error}`
