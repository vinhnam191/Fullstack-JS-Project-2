...
orderRoute.get("/", async (_req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({ message: "Get list of oders successfully", data: oders })
    } catch (error) {
...
orderRoute.get("/user/:userId", auth, async (req: Request, res: Response) => {
    try {
        ...
        if (userId && (typeof userId == 'number')) {
            ...
            res.status(200).json({ message: "Get orders info by user successfully", data: orders })
        } else {
...
orderRoute.get("/user/:userId/order", auth, async (req: Request, res: Response) => {
    try {
        ...
        if (userId && (typeof userId == 'number')) {
            ...
            res.status(200).json({ message: "Get orders complete by user successfully", data: orders })
        } else {
            res.status(400).send({ message: "Please pass user id as a number" })
        }
    ...
    try {
        ...
        if (newOrder.product_id && newOrder.user_id && newOrder.quantity) {
            if (newOrder.quantity <= 0) {
                res.status(400).send({ message: "Please input quantity of product as positive numbers" })
            }
...
orderRoute.put("/:id", auth, async (req: Request, res: Response) => {
    try {
        ...
        if (orderId && (typeof orderId == 'number')) {
            ...
            res.status(200).json({ message: "Update status of order to complete successfully", data: orders })
        } else {
