...
userRoute.get("/", auth, async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(200).json({ message: "Get list of users successfully", data: users })
    } catch (error) {
...
userRoute.get("/:id", auth, async (_req: Request, res: Response) => {
    try {
        ...
        if (userId && (typeof userId == 'number')) {
            ...
            res.status(200).json({ message: "Get user info successfully", data: user })
            return user;
        } else {
            res.status(400).send({ message: "Invalid userId or user id must be a number" })
        }
...
userRoute.post("/signUp", async (req: Request, res: Response) => {
    try {
        ...
        if (newUser.userName && newUser.password && newUser.fistName && newUser.lastName) {
...
userRoute.post("/authenticate", async (req: Request, res: Response) => {
    try {
        ...
        if (userName && password) {
            ...
            res.status(400).send({ message: "Please input username, password to login" })
        }
    } catch (error) {
        res.status(403).json({
            message: `${error}`,
